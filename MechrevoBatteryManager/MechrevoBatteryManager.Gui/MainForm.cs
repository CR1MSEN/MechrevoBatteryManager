...
namespace MechrevoBatteryManager.Gui
{
    public sealed class MainForm : Form
    {
        ...
        private void ReadEc() { Guard(delegate { var r = ThresholdManager.Read(dllPath.Text); status.Text = English ? string.Format("EC upper={0}, lower={1}, gate (0x0742)={2}, states 0x07C3/0x0770={3}/{4}, stored limit (0x087F & 0x7F)={5}, effective limit_enabled={6}.", r.BeforeUpper, r.BeforeLower, r.ChargeLimitGate, r.State07C3, r.State0770, r.StoredLimit, r.LimitEnabled) : string.Format("EC 上限={0}，下限={1}，启用门 (0x0742)={2}，状态 0x07C3/0x0770={3}/{4}，存储上限 (0x087F & 0x7F)={5}，最终 limit_enabled={6}。", r.BeforeUpper, r.BeforeLower, r.ChargeLimitGate, r.State07C3, r.State0770, r.StoredLimit, r.LimitEnabled); }); }
